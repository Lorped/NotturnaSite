<?php
	...
	$vamp=0;
		...
		$Mysql="SELECT fdv,fdvmax,lastfdv FROM personaggio WHERE idutente=$id";
...
if ($vamp == 1) {
	...
	$zona = $res['zona'];
	// PF
	$Mysql="SELECT * from discipline where iddisciplina=12 and idutente=$id";
	$Result=mysql_query($Mysql);
	if ( $res=mysql_fetch_array($Result)) {
		$robustezza=$res['livello'];
	}
	$Mysql="SELECT * from skill where idskill=28 and idutente=$id";
	$Result=mysql_query($Mysql);
	if ( $res=mysql_fetch_array($Result)) {
...
?>
		<tr>
			<td class="val"><?=$res['nomedisc']?>
				<?	$idd=$res['iddisciplina'];
					$Mysql2="SELECT * from poteri LEFT join poteri_main on poteri.idpotere=poteri_main.idpotere where poteri_main.iddisciplina=$idd and idutente=$id";
...
<?
			...
			$MySql = "SELECT  nomerituale ,livello FROM rituali_t
          		...
          		WHERE idutente = '$id' ORDER BY livello ASC";
			$Results = mysql_query($MySql);
			if (mysql_errno())  die ( mysql_errno().": ".mysql_error() );
			$num_rows = mysql_num_rows($Results);
...
<?
		...
		$Mysql="SELECT * FROM skill_main LEFT JOIN skill ON skill_main.idskill = skill.idskill AND skill.idutente = $id WHERE tipologia = 0 ORDER BY nomeskill";
		$Result=mysql_query($Mysql);
...
<?
		...
		$Mysql="SELECT * FROM skill_main LEFT JOIN skill ON skill_main.idskill = skill.idskill AND skill.idutente = $id WHERE tipologia = 1 ORDER BY nomeskill";
		$Result=mysql_query($Mysql);
		for ( $i=0; $i < floor($num_rows/3) ;$i++)  {
			$res=mysql_fetch_array($Result);
			$liv = $res['livello'];
...
<?
  ...
  //contatti
	   		$MySql = "SELECT  nomecontatto, livello  FROM contatti WHERE idutente = $id ";
...
<?
			...
			$MySql = "SELECT  nomepregio ,valore FROM pregidifetti
          		...
          		WHERE idutente = '$id' ORDER BY valore DESC";
			$Results = mysql_query($MySql);
			if (mysql_errno())  die ( mysql_errno().": ".mysql_error() );
			$num_rows = mysql_num_rows($Results);
...
?>
		<tr>
			<td colspan=9 class="alc title2"><hr>Bio e note<hr></td>
		</tr>
		<tr>
			<td colspan=9 ><?=$bio?></td>
		...
		<tr>
		<tr>
			<td colspan=9 ><b>Note</b><br><?=$note?><br><?=$notemaster?></td>
